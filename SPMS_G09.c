...
typedef struct {
    char type[MAX_NAME_LENGTH]; // Type of booking (ing, Reservation, Event, Essentials)
...
// Array for Booking timeslots
//bool timeslot[MAX_PARKING_SLOTS][31][24] ={0};
...
forPri addforpri(char *type, char *date, char *time, float duration, char facilities[][MAX_NAME_LENGTH], int facility_count, int booking_id){
    ...
    for (i=0; i<facility_count; i++){
        ...
        }else if (strcmp(facilities[i], "valet")==0){
            newforpri.facilities[i] = 5;continue;
        }else if (strcmp(facilities[i], "inflation")==0){
...
void addBatch(const char *filename) {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        while ((token = strtok(NULL, " \n")) != NULL && facility_count < MAX_FACILITIES) {
            char faclist[6][MAX_NAME_LENGTH] = {"battery", "cable", "locker", "umbrella", "valet", "inflation"};
            int i, j=0;
            for (i=0;i<6;i++){
...
void all(int pipes[MAX_MEMBER][2][2], bool timetable[facnum][MAX_PARKING_SLOTS][days][hour], Booking bookings[MAX_BOOKINGS], int book_num, FILE *file){
    printf("\n*** Booking Manager - Summary Report ***\n\n");
    printf("Performance:\n\n");
    fprintf(file, "\n*** Booking Manager - Summary Report ***\n\n");
...
void printBookings(const char *algorithm) {
    ...
    if (strcmp(algorithm, "fcfs") == 0) {
        ...
        sleep(1);
        printf("*** Booking - ACCEPT / FCFS ***\n");
        displayaccept(pipes, bookings);
        sleep(1);
        printf("*** Booking - REJECTED / FCFS ***\n");
        ...
        sleep(1);
        printf("*** Booking - ACCEPT / PRIO ***\n");
        displayaccept(pipes, bookings);
        sleep(1);
        printf("*** Booking - REJECTED / PRIO ***\n");
        displayreject(pipes, bookings);
    }
        ...
        sleep(1);
        printf("*** Booking - ACCEPT / OPTI ***\n");
        displayaccept(pipes, bookings);
        sleep(1);
        printf("*** Booking - REJECTED / OPTI ***\n");
        displayreject(pipes, bookings);
    }
...
void Input(char *input) {
    ...
    if (strcmp(command, "addParking") == 0) {
        ...
        if(!facerrorhandle(facilities, facility_count)){
            printf("Error: Invalid facility provided. Valid facilities are: battery, cable, locker, umbrella, valet, inflation.\n");
            return;
        }
        addParking(member, date, time, duration, facilities, facility_count);
        printf("-> [Pending]\n");
